...
export const customHandlers = () => {
  ...
  const customDrawHandlers = {
    [CustomTypes.CUSTOM_GLOWING_STAR]: (
      ...
      context.ellipse(x, y, radius, 3 * radius, rotation, startAngle, endAngle);
      context.fill();
      context.restore();
      context.fillStyle = 'yellow';
      context.ellipse(x, y, 3 * radius, radius, rotation, startAngle, endAngle);
      context.fill();
    },
  };
  ...
  const objects = [
    { type: objectTypes.RECT, x: 300, y: 300, width: 600, height: 600, contextProps: { fillStyle: 'black' } },
